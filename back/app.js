...
const verifyToken = (req, res, next) => {
  ...
  try {
    const data = jwt.verify(req.headers.authorization.replace('Bearer ', ''), jwtSecret)
...
const verifyRefreshToken = (req, res, next) => {
  ...
  try {
    ...
    console.error(error)
    if (error.name === 'TokenExpiredError') {
      return res.status(419).json({ message: '만료된 리프레시 토큰입니다.', code: 'expired' })
    }
...
app.post('/refreshToken', verifyRefreshToken, (req, res, next) => {
  const accessToken = jwt.sign({ sub: 'access', email: res.locals.email }, jwtSecret, { expiresIn: '5m' })
...
app.post('/login', (req, res, next) => {
  ...
  const refreshToken = jwt.sign({ sub: 'refresh', email: req.body.email }, jwtSecret, { expiresIn: '24h' })
  const accessToken = jwt.sign({ sub: 'access', email: req.body.email }, jwtSecret, { expiresIn: '5m' })
...
app.post('/accept', verifyToken, (req, res, next) => {
  ...
  if (order.rider) {
    return res.status(400).json({ message: '다른 사람이 이미 수락한 주문건입니다. ' })
  }
...
app.post('/complete', verifyToken, upload.single('image'), (req, res, next) => {
  ...
  const order = orders.find(v => v.orderId === req.body.orderId && v.rider === res.locals.email)
  if (!order) {
    return res.status(400).json({ message: '유효하지 않은 주문입니다.' })
  }
...
app.get('/showmethemoney', verifyToken, (req, res, next) => {
  const order = orders.filter(v => v.rider === res.locals.email && !!v.completedAt)
  res.json({
